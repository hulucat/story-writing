...
title_llm = ChatOpenAI(
    model=glm_model_name, metadata={"name": "title_llm"}, api_key=glm_api_key, base_url=glm_base_url
)
chapter_title_llm = ChatOpenAI(
    model=glm_model_name, metadata={"name": "chapter_title_llm"}, api_key=glm_api_key, base_url=glm_base_url
)
summary_llm = ChatOpenAI(
    model=glm_model_name, metadata={"name": "summary_llm"}, api_key=glm_api_key, base_url=glm_base_url
)
brainstorm_llm = ChatOpenAI(
...
plan_llm = ChatOpenAI(model=glm_model_name, metadata={"name": "plan_llm"}, api_key=glm_api_key, base_url=glm_base_url)
write_llm = ChatOpenAI(
    model=glm_model_name, metadata={"name": "write_llm"}, api_key=glm_api_key, base_url=glm_base_url
)
...
brainstorm_messages = [
    (
        "system",
        "You are an assistant tasked with brainstorming ideas for brainstorming ideas for \
...
outline_messages = [
    (
        "system",
        "You are an assistant tasked with outlining a new chapter in a story. You will be provided with \
...
write_messages = [
    (
        "system",
        "You are an assistant tasked with writing book chapters in simplified Chinese. You will receive an outline \
...
def write_chapter(user_message, chapters_summary, state):
    brainstorm_ideas = brainstorm_chain.invoke(
        {
            "story_summary": chapters_summary,
            "action": user_message,
            "summary_request": state["summary_request"],
            "detail_request": state["detail_request"],
            "style_request": state["style_request"],
        }
    )
    ...
    chapter_title = chapter_title_llm.invoke(
...
def get_title(state, first_chapter):
    return title_llm.invoke(
        f"Please come up with a short title, less than 6 words, for a story. The story has the following overall plot {state['summary']}, and here is the first chapter {first_chapter}"
    ).content.replace('"', "")
...
def write_first_chapter(state):
    ...
    state["chapter_graph"] = {
        "1": Chapter(content=chapter_content, title=chapter_title, children=[], siblings=[], cousins=[], parent="-1")
    }
...
def edit_chapter(state):
    ...
    # create new chapter
    state["chapter_graph"][str(int(state["current_chapter_id"]) + 1)] = Chapter(
        content=chapter_content,
        title=chapter_title,
        children=[],
        siblings=deepcopy(
            state["chapter_graph"][state["chapter_id_viewing"]]["siblings"] + [state["chapter_id_viewing"]]
        ),
